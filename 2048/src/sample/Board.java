...
public class Board extends Group {
    ...
    public static final int GRID_WIDTH = 5*CELL_SIZE+2*BORDER_WIDTH;
    ...
    private void createScore() {
        ...
        if(Game2048.STEP>=3){
            Label lblTitle=new Label("Cat");
            Label lblSubtitle=new Label("2048");
            ...
            HBox hTime = new HBox();
            hTime.setMinSize((double)this.GRID_WIDTH, 50.0D);
            hTime.setAlignment(Pos.BOTTOM_RIGHT);
            this.lblTime.getStyleClass().addAll(new String[]{"game-label", "game-time"});
            this.lblTime.textProperty().bind(this.clock);
            this.timer = new Timeline(new KeyFrame[]{new KeyFrame(Duration.ZERO, (e) -> {
                this.clock.set(LocalTime.now().minusNanos(this.time.toNanoOfDay()).format(this.fmt));
            }, new KeyValue[0]), new KeyFrame(Duration.seconds(1.0D), new KeyValue[0])});
            this.timer.setCycleCount(-1);
            hTime.getChildren().add(this.lblTime);
            this.vGame.getChildren().add(hTime);
